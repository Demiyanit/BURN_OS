...
#define PIC2_COMMAND_PORT     0x20
#define PIC2_DATA_PORT        0x21

/**
 * Initialization Contrl Word 1
 * ----------------------------
 *  0   IC4     if set, the PIX expects to recieve ICW4 during initialization
 *  1   SGNL    if set, only 1 PIC in the system; if unset, the PIC is cascaded with slave PICs
 *              and ICW3 must be sent to controller
 * 
 *  2   ADI     call address interval, set 4, not set: 8; ignored on x86 if set to 0
 *  3   LTIM    if set, 
...
void i8259_Configure(uint8_t offsetPic1, uint8_t offsetPic2, bool autoEoi) {
    ...
    // mask all interrupts untill they are enabled by the device driver
